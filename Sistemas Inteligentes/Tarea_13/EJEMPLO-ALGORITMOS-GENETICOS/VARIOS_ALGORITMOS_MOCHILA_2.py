# -*- coding: utf-8 -*-
"""
Created on Wed Aug 14 10:48:58 2024

@author: Jaime
"""
#==============================================================================
# PROBLEMA DE LA MOCHILA RESUELTO CON 10 ALGORITMOS Y 10 ARTÍCULOS

# Explicación dEL CÓDIGO
# 1. Generación de datos simulados: Se crea un conjunto de 20 objetos con pesos 
# yvalores aleatorios.

# 2. Implementación de algoritmos: Se implementan las diez soluciones al problema 
# de la mochila.
...
# Resultados:
# 1. Después de ejecutar este código, obtendrá dos gráficos:
...
# Paso 1: Generar datos simulados con 20 artículos
np.random.seed(42)
num_items = 17
...
def get_selected_items(solution, num_items):
...
def backtracking_knapsack_wrapper(weights, values, capacity):
    value, selected_items = backtracking_knapsack(weights, values, capacity, len(weights))
    selection_vector = get_selected_items(selected_items, len(weights))
    return value, selection_vector
...
def branch_and_bound_knapsack(weights, values, capacity):
    ...
    while Q:
        ...
        if v.bound > max_value:
            Q.append(Node(v.level, v.value, v.weight, v.bound, v.items))
    selected_items = get_selected_items(best_items, n)
    return max_value, selected_items
...
def integer_programming_knapsack(weights, values, capacity):
    ...
    prob.solve()
    selected_items = [int(pulp.value(x[i])) for i in range(n)]
...
def genetic_algorithm_knapsack(weights, values, capacity, population_size=50, generations=100):
    ...
    selected_items = get_selected_items(best_individual, len(weights))
...
def simulated_annealing_knapsack(weights, values, capacity, temp=10000, cooling_rate=0.99, stopping_temp=0.1):
    ...
    selected_items = get_selected_items(best_solution, len(weights))
...
def aco_knapsack(weights, values, capacity, iterations=100, num_ants=50, alpha=1, beta=1, rho=0.5):
    ...
    selected_items = get_selected_items(best_solution, n)
    return best_value, selected_items
...
def pso_knapsack(weights, values, capacity, population_size=50, iterations=100):
    ...
    selected_items = get_selected_items(global_best_position, n)
...
knapsack_algorithms = {
    ...
    "Backtracking": backtracking_knapsack_wrapper,
...
times = []
validity = []
solutions = pd.DataFrame(index=range(num_items), columns=knapsack_algorithms.keys())
...
for name, algorithm in knapsack_algorithms.items():
    ...
    is_valid = is_valid_solution([i for i, x in enumerate(selected_items) if x == 1], weights, capacity)
    validity.append(is_valid)
    results.append(value)
    times.append(exec_time)
    validity_status = "Válido" if is_valid else "No válido"
    solutions[name] = pd.Series(selected_items, index=solutions.index)
    print(f"{name}: Valor máximo = {value}, Tiempo de ejecución = {exec_time:.4f} segundos, Solución {validity_status}")

# Añadir una fila para el valor máximo obtenido por cada algoritmo
solutions.loc['Valor Máximo'] = results
...
# Mostrar la tabla de soluciones
print("\nTabla de Soluciones:")
print(solutions)
